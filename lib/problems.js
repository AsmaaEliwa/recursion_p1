...
function reverseString(str) {
    ...
    arr=str.split("")
    return arr.pop() +reverseString(arr.join("")) 
...
function flatten(data) {
    if (!Array.isArray(data)) {
        return [data]; // Wrap the single value in an array
      }
    let holder=[]
    data.forEach(ele=>{
        if(Array.isArray(ele)){
             const flattenele=flatten(ele)
             holder.push(...flattenele)
        }else{
            holder.push(ele)
        }
    })
...
function fileFinder(directories, targetFile) {
    // Base case: If directories is null or undefined, return false.
    if (!directories) {
      return false;
    }
